...
- ROS 是一个以网络为中心的框架，即整个 ROS 应用系统的的各个节点可以分布在网络互联的多个设备上。例如摄像机节点运行在机器人本地的计算机上，此时 RViz 等节点可以运行在互联网上的其他设备上。采用此方法可以将负载分布在多台计算机上。
...
- ROS 可以使用 OpenCV 处理图像数据。其中包含了大量的视觉算法和机器学习方法。在使用 OpenCV 作为图像处理库时，需要将 ROS 获取的图像数据进行转换，该转换使用 `cv_bridge` 包 ([Wiki页面](http://wiki.ros.org/cv_bridge)) 完成。

- ROS 中的图像数据的格式为 `sensor_msgs/Image` 。

- ROS 的摄像机坐标系与底座的坐标系不同，摄像机指向外的方向为 `z` 轴，垂直于图像平面，图像平面中， `x` 轴水平指向右， `y` 轴竖直向下。

#### Note: 如何寻找第三方 ROS 数据包
- 寻找 ROS Wiki
